	...
	if(current_user_can('administrator')){
		...
		if(!is_wp_error($response)){
        	...
        	if(isset($headers['location'])){
        		...
        		// old nextcloud response
        		$expected_response = esc_attr($nextcloud_url).'login/flow?clientIdentifier='.esc_attr($identifier);

        		// also check newer nextcloud response
        		if(($response && str_contains($response, esc_attr($nextcloud_url).'login/flow?') && str_contains($response, 'clientIdentifier='.esc_attr($identifier))) || ($response == $expected_response)){
