...
use RexTheme\RexShoppingRakuten\Node;
...
//        }
    ...
    {
        $node = new Node('product type');
        $brand = $this->safeCharEncodeText($productType);
        $this->nodes['product type'] = $node->value($productType)->_namespace($this->namespace);
    }
...
//    }
    ...
    {
        $node = new Node('age group');
        $this->nodes['age group'] = $node->value($ageGroup)->_namespace($this->namespace);
    }
