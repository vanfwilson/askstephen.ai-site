...
class WPLA_RepricingPage extends WPLA_Page {
	...
	public function resubmitPnqUpdateForSelectedItems() {
		...
		$item_ids = wpla_clean($_REQUEST['listing']);
	...
	public function applyMinMaxPrices() {
		...
		$item_ids = ! empty($_REQUEST['item_ids']) ? explode( ',', wpla_clean($_REQUEST['item_ids']) ) : array();
	...
	public function displayRepricingPage() {
		...
		if ( $this->requestAction() == 'wpla_resubmit_all_failed_prices' ) {
			...
			$this->showMessage( sprintf( __( '%s failed prices were scheduled for resubmission.', 'wp-lister-for-amazon' ), count($items) ) );
		}
