...
class WPLA_LogPage extends WPLA_Page {
	...
	public function onWpAdminMenu() {
		...
		if ( current_user_can('manage_amazon_options') && ( self::getOption( 'log_to_db' ) == '1' ) ) {
			add_submenu_page( self::ParentMenuId, $this->getSubmenuPageTitle( 'Logs' ), __( 'Logs', 'wp-lister-for-amazon' ), 
	...
	public function handleSubmit() {
		...
		if ( $this->requestAction() == 'wpla_display_log_entry' ) {
		    $this->displayLogEntry( wpla_clean($_REQUEST['log_id']) );
			exit();
		}
		...
		if ( $this->requestAction() == 'wpla_clear_amazon_log' ) {
			...
			$this->clearLog();
			$this->showMessage( __( 'Database log has been cleared.', 'wp-lister-for-amazon' ) );
		}
