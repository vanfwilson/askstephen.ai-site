...
class WPLA_ReportsTable extends WP_List_Table {
    ...
    function column_details($item){
        ...
        $actions = array(
            ...
            'process_amazon_order_data'  => sprintf('<a href="?page=%s&action=%s&report_id=%s&_wpnonce=%s">%s</a>',$page,'process_order_data_report',$item['id'], wp_create_nonce( 'wpla_process_report' ), __( 'Process Order Report', 'wp-lister-for-amazon' )),
        ...
        if ( $item['ReportType'] != 'GET_FLAT_FILE_ORDER_REPORT_DATA_INVOICING' ) unset( $actions['process_amazon_order_data'] );
        ...
        $report_is_enabled = in_array( $item['ReportProcessingStatus'], ['_DONE_', 'DONE'] ) ? $report_is_enabled : false;

        if ( ! $report_is_enabled ) {
            $title = "<span style='color:silver'> $title </span>";
            unset( $actions['process_amazon_report'] );  
            unset( $actions['process_amazon_order_data'] );
        ...
        if ( !in_array( $item['ReportType'], array( '_GET_AFN_INVENTORY_DATA_', '_GET_FBA_MYI_UNSUPPRESSED_INVENTORY_DATA_', 'GET_AFN_INVENTORY_DATA', 'GET_FBA_MYI_UNSUPPRESSED_INVENTORY_DATA' ) ) ) {
            unset( $actions['process_fba_report'] );  
        }
        if ( !in_array( $item['ReportType'], [ '_GET_MERCHANT_LISTINGS_DATA_', 'GET_MERCHANT_LISTINGS_DATA' ] ) ) {
            unset( $actions['process_amazon_report'] );  
        }
        if ( !in_array( $item['ReportType'], [ 'GET_FLAT_FILE_ORDER_REPORT_DATA_INVOICING' ] ) ) {
            unset( $actions['process_amazon_order_data'] );
        }
        if ( !in_array( $item['ReportType'], [ '_GET_MERCHANT_LISTINGS_DEFECT_DATA_', 'GET_MERCHANT_LISTINGS_DEFECT_DATA']) ) {
            unset( $actions['process_quality_report'] );  
        }
        if ( !in_array( $item['ReportType'], [ '_GET_AMAZON_FULFILLED_SHIPMENTS_DATA_', 'GET_AMAZON_FULFILLED_SHIPMENTS_DATA_GENERAL'] ) ) {
            unset( $actions['process_fba_shipment_report'] );  
        }
        if ( !in_array( $item['ReportType'], [ '_GET_FBA_FULFILLMENT_INVENTORY_HEALTH_DATA_', 'GET_FBA_FULFILLMENT_INVENTORY_HEALTH_DATA' ] ) ) {
            unset( $actions['process_fba_inv_age_report'] );  
        }
